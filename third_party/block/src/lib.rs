...
/// Opaque stand-in for an Objective-C class.
///
/// Upstream declares this as an empty — and therefore uninhabited — enum, which makes
/// `static _NSConcreteStackBlock: Class` a static of an uninhabited type. rustc is phasing
/// that out (rust-lang/rust#74840) because such a static can never be read. Nothing here
/// ever reads it: only its address is taken, as the `isa` pointer of a stack block. An
/// inhabited zero-sized opaque struct is the ordinary FFI spelling for that and keeps the
/// address semantics identical.
#[repr(C)]
struct Class {
    _private: [u8; 0],
}
